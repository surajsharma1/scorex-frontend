          ...
          {activeTab === 'settings' && (
            <div className="p-8">
              ...
              <div className="mt-12 p-8 bg-red-500/5 border border-red-500/20 rounded-3xl">
                <h4 className="text-2xl font-black mb-6 flex items-center gap-3" style={{ color: 'var(--text-primary)' }}>
                ...
                <p className="text-lg mb-8 max-w-2xl" style={{ color: 'var(--text-secondary)' }}>
                  Delete this club. <span className="font-bold text-red-400">This action cannot be undone</span>. This will permanently delete the club, all data, and remove all members.
                </p>
                <button
                  onClick={async () => {
                    if (!confirm('Are you absolutely sure? This will delete the club permanently.')) return;
                    try {
                      await clubAPI.deleteClub(id!);
                      addToast({ type: 'success', title: 'Club Deleted', message: 'Club deleted successfully' });
                      navigate('/clubs?tab=my');
                    } catch (error: any) {
                      addToast({ type: 'error', title: 'Error', message: error.response?.data?.message || 'Failed to delete' });
                    }
                  }}
                  className="px-8 py-4 bg-gradient-to-r from-red-500 to-red-600 text-white font-bold rounded-2xl hover:shadow-lg hover:shadow-red-500/25 transform hover:scale-[1.02] transition-all duration-200 text-lg"
                >
