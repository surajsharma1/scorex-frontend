...
export default function LiveScoring() {
  ...
  // Decision pending — locks scoring buttons, fires overlay animation
  ...
  const handlePlayersDone = async (players: any) => {
    ...
    try {
      ...
      await fetchMatch();
      // Fire NEW_BOWLER animation when a bowler change is confirmed
      if ((selectContext.reason === 'bowler_change' || selectContext.reason === 'over_end') && players.bowler) {
        fireTrigger('NEW_BOWLER', { bowler: players.bowler, overs: '0.0', runs: 0, wickets: 0 }, 8);
      }
      setStep('scoring');
      setPanel('main');
  ...
  const handleRetirement = async (role: 'striker' | 'nonStriker') => {
    ...
    const retiredStats = role === 'striker' ? activeStriker : activeNonStriker;
    if (!retiredName || submitting) return;
    // Fire retired animation BEFORE submitting so overlay shows it immediately
    fireTrigger('BATSMAN_CHANGE', {
      outName: retiredName,
      howOut: 'Retired Hurt',
      outRuns: retiredStats?.runs || 0,
      outBalls: retiredStats?.balls || 0,
      inName: '',
      isSub: false,
    }, 8);
    setSubmitting(true);
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ background: N.bg, color: N.textPrimary }}>
      ...
      {/* Decision Pending — locks all scoring, shows overlay animation in broadcast */}
      {isDecisionPending && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center" style={{ background: 'rgba(0,0,0,0.75)', backdropFilter: 'blur(4px)' }}>
          <div className="text-center p-8 rounded-3xl mx-4" style={{ border: `2px solid ${N.amber}`, background: N.bgCard, boxShadow: `0 0 60px ${N.amberDim}` }}>
            <AlertTriangle className="w-16 h-16 mx-auto mb-4 text-amber-500 animate-pulse" />
            <h2 className="text-2xl font-black text-amber-500 mb-2 uppercase tracking-widest">Decision Pending</h2>
            <p className="text-sm mb-6" style={{ color: N.textMuted }}>3rd Umpire Review — Scoring locked</p>
            <button onClick={toggleDecisionPending}
              className="px-8 py-3 bg-amber-500 text-black font-black rounded-2xl hover:scale-105 transition-all">
              Resume Scoring
            </button>
          </div>
        </div>
        ...
        {panel === 'main' && (
          <div className="p-4 space-y-4">
            ...
            <div>
              ...
              {[0, 1, 2, 3, 4, 6].map(r => (
                  <button
                    disabled={submitting || isDecisionPending}
                    onClick={() => submitBall({ runs: r })}
                    className="py-5 rounded-2xl font-black text-2xl transition-all active:scale-95 disabled:opacity-40 shadow-lg disabled:cursor-not-allowed"
