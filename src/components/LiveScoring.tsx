...
function BroadcastPanel({ fire, match }: any) {
  ...
  const triggers = [
    ...
    { label: '↩ Restore State', fn: () => fire('RESTORE') },
  ];
...
export default function LiveScoring() {
  ...
  const handleRetirement = (type: 'striker' | 'nonStriker') => {
    ...
    // Fire retired animation overlay before changing players
    fireTrigger('WICKET', { playerName: retiredName, outType: 'Retired Hurt', runs: type === 'striker' ? (activeStriker?.runs || 0) : (activeNonStriker?.runs || 0), isRetired: true }, 6);
