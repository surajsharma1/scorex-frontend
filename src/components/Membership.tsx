...
function OverlayPreviewModal({ isOpen, onClose, level, templates }: OverlayPreviewModalProps) {
  ...
  // Recompute scale on container resize — debounced via rAF to avoid
  // rapid setState on mobile (iOS/Android resize viewport on every scroll
  // as address bar shows/hides, causing ResizeObserver to fire continuously).
  useEffect(() => {
    if (!isOpen) return;
    let rafId: number;
              ...
              >
                {selected ? (
                  <iframe
                    ...
                    key={iframeKey}
                    src={`/overlays/${selected.file || (selected.id + '.html')}?demo=true`}
