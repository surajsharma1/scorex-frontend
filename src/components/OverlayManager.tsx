...
import { 
  Eye, Plus, Save, Trash2, Edit, Copy, RefreshCw, 
  ExternalLink, X, AlertCircle, PlaySquare, Target, 
  Activity, ShieldAlert, Timer, Maximize2, Smartphone 
} from 'lucide-react';
...
export default function OverlayManager({ tournamentId, matches }: { tournamentId?: string, matches?: any[] }) {
  ...
  // Ultra-Robust Animation Trigger: Broadcasts to all layers and inner iframes
  const triggerAnim = (event: string) => {
    const payload = { type: 'OVERLAY_ACTION', payload: { event } };
    
    // Broadcast to top window
    window.postMessage(payload, '*');
    window.dispatchEvent(new CustomEvent('OVERLAY_ACTION', { detail: { event } }));

    // Penetrate any hidden iframes generated by the preview renderer
    ...
    <div className="space-y-8 mt-4 mx-2">
      ...
      {isMobileFullscreen && activePreview && (
        <div className="fixed inset-0 z-[999] bg-black flex flex-col animate-in fade-in duration-200">
          ...
          {/* Scrollable Triggers Bottom Bar */}
          <div className="p-4 bg-[#0a0a0f] border-t border-gray-800 flex overflow-x-auto hide-scrollbar gap-3 shrink-0">
             ...
             <button onClick={() => triggerAnim('DECISION PENDING')} className="px-6 py-3.5 bg-amber-500/10 text-amber-500 font-bold border border-amber-500/30 rounded-xl whitespace-nowrap shrink-0 active:scale-95 transition-transform">DP</button>
          </div>
