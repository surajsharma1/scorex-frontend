    ...
    <div className="flex flex-col h-screen" style={{ background: 'var(--bg-primary)', color: 'var(--text-primary)' }}>
      ...
      <div className="h-14 flex items-center justify-between px-2 sm:px-4 shrink-0 shadow-md relative z-10"
        ...
        <div className="flex items-center gap-2 min-w-0">
          ...
          <button
            className="md:hidden p-2 rounded-lg bg-gray-900 border border-gray-800 text-gray-400 hover:text-white flex-shrink-0"
        ...
        <div className="flex items-center gap-1.5 flex-shrink-0">
          ...
          <div className="flex items-center gap-0.5 bg-gray-900 border border-gray-800 rounded-lg p-1">
            <button onClick={() => setZoom(z => Math.max(0.3, z * 0.8))} className="p-1.5 rounded text-gray-400 hover:text-white hover:bg-gray-800 transition-all">
              <ZoomOut className="w-3.5 h-3.5" />
            </button>
            <span className="text-xs font-bold text-gray-400 w-10 text-center tabular-nums">{Math.round(zoom * 100)}%</span>
            <button onClick={() => setZoom(z => Math.min(4, z * 1.25))} className="p-1.5 rounded text-gray-400 hover:text-white hover:bg-gray-800 transition-all">
              <ZoomIn className="w-3.5 h-3.5" />
            </button>
            <button onClick={() => setZoom(1)} className="p-1.5 rounded text-gray-400 hover:text-white hover:bg-gray-800 transition-all">
          ...
          <button
            ...
            className="px-2.5 py-1.5 bg-gray-700 hover:bg-gray-600 text-white text-xs font-bold rounded-lg transition-all flex items-center gap-1 whitespace-nowrap"
          >
      ...
      <div className="flex-1 flex overflow-hidden relative">
        ...
        {/* ── CENTER: canvas + triggers ── */}
          ...
          >
            <div
              className="relative bg-black rounded-xl border border-gray-800 shadow-2xl overflow-hidden w-full"
          ...
          <div className="shrink-0 border-t overflow-y-auto" style={{ maxHeight: '45vh', backgroundColor: 'var(--bg-card)', borderColor: 'var(--border)' }}>
            ...
            {TRIGGER_GROUPS.map(group => (
              <div key={group.label}>
                <button
                  ...
                  className="w-full flex items-center justify-between px-4 py-2.5 hover:bg-gray-900 transition-all border-b border-gray-900 sticky top-0 bg-[#0a0a0f] z-10"
                >
            ...
            <div className="px-3 py-2 border-t border-gray-800 sticky bottom-0 bg-[#0a0a0f]">
              <button
                onClick={handleRestore}
                className="w-full py-2.5 bg-gray-800 hover:bg-gray-700 text-white text-xs font-black rounded-lg transition-all flex items-center justify-center gap-2"
              >
