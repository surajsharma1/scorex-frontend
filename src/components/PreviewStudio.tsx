...
import {
  ...
  ZoomIn, ZoomOut, RotateCcw, ChevronDown, ChevronUp, Menu, Sun, Moon
} from 'lucide-react';
...
// Level 1: only 4, 6, OUT
const LVL1_TRIGGERS = [
  { label: 'FOUR (4)', type: 'FOUR', data: {}, color: 'cyan' },
  { label: 'SIX (6)', type: 'SIX', data: {}, color: 'cyan' },
  { label: 'WICKET', type: 'WICKET', data: {}, color: 'red' },
];

// Level 2: full trigger groups
const TRIGGER_GROUPS = [
  {
    label: 'MATCH STARTUP', color: 'blue',
    triggers: [
      { label: 'VS Screen', type: 'VS_SCREEN', data: { team1: 'MUMBAI', team2: 'CHENNAI' } },
      { label: 'Toss Result', type: 'SHOW_TOSS', data: { tossWinnerName: 'MUMBAI', tossDecision: 'bat' } },
      { label: 'Innings Intro', type: 'START_INNINGS_INTRO', data: { striker: 'R. Sharma', nonStriker: 'H. Pandya', bowler: 'P. Cummins' } },
    ],
  },
  {
  ...
  },
  {
    ...
    triggers: [
      { label: 'Batsman Profile', type: 'BATSMAN_PROFILE', data: { playerName: 'R. Sharma', runs: 82, balls: 45, fours: 8, sixes: 3, sr: '182.2' } },
      { label: 'Bowler Profile', type: 'BOWLER_PROFILE', data: { playerName: 'P. Cummins', overs: '3.4', wickets: 1, runs: 34, economy: '10.2' } },
      { label: 'Wicket Out', type: 'WICKET_SWITCH', data: { outName: 'R. SHARMA', outRuns: 82, outBalls: 45, howOut: 'c Dhoni b Jadeja', inName: 'S. YADAV' } },
      { label: 'New Bowler', type: 'NEW_BOWLER', data: { bowler: 'J. BUMRAH', overs: '0.0', wickets: 0, runs: 0 } },
    ],
  },
  {
    label: 'FULL SCREEN', color: 'green',
    triggers: [
      { label: 'Innings Break', type: 'INNINGS_BREAK', data: { chasingTeam: 'CHENNAI', targetScore: 214 } },
      ...
      { label: 'Match End', type: 'MATCH_END', data: { winnerTeam: 'MUMBAI INDIANS', winMargin: 'Won by 6 wickets' } },
    ],
  },
];
...
const COLOR_MAP: Record<string, string> = {
  ...
  red:    'bg-red-500/10 border-red-500/30 text-red-400 hover:bg-red-500 hover:text-white',
};
...
export default function PreviewStudio() {
  ...
  // Derived: are we currently showing a level 1 overlay?
  const isLevel1 = selectedTemplate.level === 1;
        ...
        <div className="flex-1 flex flex-col overflow-hidden min-w-0"
          ...
          >
            <div
              ...
              style={{ aspectRatio: '16/9', maxHeight: '100%', borderColor: 'var(--border)' }}
            >
              <iframe
                ref={iframeRef}
                id="preview-frame"
          ...
          {/* ── TRIGGER PANEL ── */}
          <div className="shrink-0 border-t" style={{ backgroundColor: 'var(--bg-card)', borderColor: 'var(--border)' }}>

            {isLevel1 ? (
              /* Level 1: flat row of 3 buttons only */
              <div className="px-4 py-3 flex items-center gap-3 flex-wrap">
                <span className="text-[10px] font-black uppercase tracking-widest mr-1" style={{ color: 'var(--text-muted)' }}>LIVE ACTION</span>
                {LVL1_TRIGGERS.map(trigger => (
                  <button
                    key={trigger.type}
                    onClick={() => fireTrigger(trigger.type, trigger.data)}
                    className={`px-4 py-2 border rounded-lg font-black text-xs transition-all active:scale-95 ${COLOR_MAP[trigger.color]}`}
                  >
                    {trigger.label}
                  </button>
                ))}
              ...
              /* Level 2: full grouped trigger panel */
              <div className="overflow-y-auto" style={{ maxHeight: '45vh' }}>
                {TRIGGER_GROUPS.map(group => (
                  <div key={group.label}>
                    <button
                      onClick={() => toggleGroup(group.label)}
                      className="w-full flex items-center justify-between px-4 py-2.5 transition-all border-b sticky top-0 z-10"
                      style={{ background: 'var(--bg-card)', borderColor: 'var(--border)' }}
                    >
                      <span className="text-[10px] font-black uppercase tracking-widest" style={{ color: 'var(--text-muted)' }}>{group.label}</span>
                ...
                {/* Restore at bottom */}
                <div className="px-3 py-2 border-t sticky bottom-0" style={{ background: 'var(--bg-card)', borderColor: 'var(--border)' }}>
                  <button
                    onClick={handleRestore}
                    className="w-full py-2.5 text-xs font-black rounded-lg transition-all flex items-center justify-center gap-2"
