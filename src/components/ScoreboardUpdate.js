...
const getTeamName = (teams, index) => teams[index]?.name || `Team ${index + 1}`;
const createDefaultBatsmen = () => [
...
export default function ScoreboardUpdate({ tournament, matchId, onUpdate }) {
    ...
    const [liveScores, setLiveScores] = useState(() => {
        ...
        const getTeamData = (teamKey, index) => ({
    ...
    useEffect(() => {
        ...
        if (battingTeamObj && battingTeamObj.players) {
            const playersArray = Array.isArray(battingTeamObj.players)
                ...
                : [];
            playersList = playersArray.map((p) => ({
    ...
    const saveScoresToBackend = useCallback(async (scores) => {
    ...
    const triggerWicketAnimation = (message) => {
    ...
    return (_jsx("div", { className: "space-y-4 max-w-md mx-auto p-2", children: !tossWinner && (_jsxs("div", { className: "bg-slate-800 p-4 rounded-xl border border-slate-700", children: [_jsxs("h3", { className: "text-lg font-semibold mb-3 flex items-center gap-2", children: [_jsx(Coins, { className: "w-5 h-5 text-yellow-400" }), " Toss"] }), _jsxs("div", { className: "grid grid-cols-2 gap-2", children: [_jsxs("button", { onClick: () => handleToss('team1', 'bat'), className: "p-2 bg-blue-600 hover:bg-blue-700 rounded-lg text-sm", children: [getTeamName(teams, 0), " Bat"] }), _jsxs("button", { onClick: () => handleToss('team2', 'bat'), className: "p-2 bg-blue-600 hover:bg-blue-700 rounded-lg text-sm", children: [getTeamName(teams, 1), " Bat"] }), _jsxs("button", { onClick: () => handleToss('team1', 'bowl'), className: "p-2 bg-red-600 hover:bg-red-700 rounded-lg text-sm", children: [getTeamName(teams, 0), " Bowl"] }), _jsxs("button", { onClick: () => handleToss('team2', 'bowl'), className: "p-2 bg-red-600 hover:bg-red-700 rounded-lg text-sm", children: [getTeamName(teams, 1), " Bowl"] })] })] })) }));
}
