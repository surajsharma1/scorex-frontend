  ...
  if (!selected) return (
    <div className="flex h-screen items-center justify-center bg-[var(--bg-primary)]">
      <div className="text-center">
        <p className="text-[var(--text-muted)] mb-4">Tournament not found.</p>
        <button onClick={() => navigate('/tournaments')} className="px-4 py-2 bg-green-500/20 text-green-400 rounded-xl font-bold">← Back</button>
      </div>
    </div>
  );
            ...
            <div className="lg:col-span-2 space-y-5">
              <div className="bg-[var(--bg-card)] p-6 rounded-2xl border border-[var(--border)]">
                <h3 className="text-sm font-black text-[var(--text-primary)] uppercase tracking-wider mb-3 flex items-center gap-2">
                  <span className="w-1 h-4 rounded-full bg-gradient-to-b from-green-400 to-emerald-600 inline-block" /> About
                </h3>
              ...
              {(selected.sponsors || []).filter((s: string) => s).length > 0 && (
                <div className="bg-[var(--bg-card)] p-6 rounded-2xl border border-[var(--border)]">
                  <h3 className="text-sm font-black text-[var(--text-primary)] uppercase tracking-wider mb-3 flex items-center gap-2">
                    <span className="w-1 h-4 rounded-full bg-gradient-to-b from-amber-400 to-orange-500 inline-block" /> Sponsors
                  </h3>
                  <div className="flex flex-wrap gap-2">
                    {(selected.sponsors || []).filter((s: string) => s).map((sp: string, i: number) => (
                      <span key={i} className="px-3 py-1.5 bg-amber-500/10 text-amber-300 text-sm font-bold rounded-xl border border-amber-500/20">{sp}</span>
                    ))}
                  </div>
                </div>
              )}
              ...
              <div className="grid grid-cols-3 gap-4">
                {[
                  ...
                  <div key={stat.label} className="bg-[var(--bg-card)] p-4 rounded-2xl border border-[var(--border)] text-center" style={{ boxShadow: `0 4px 20px ${stat.glow}` }}>
                    ...
                    <p className="text-xs text-[var(--text-muted)] font-bold uppercase tracking-wider mt-1">{stat.label}</p>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="space-y-4">
              {[
                ...
                <div key={label} className={`bg-[var(--bg-card)] p-4 rounded-2xl border border-[var(--border)] flex items-center gap-4`}>
                  ...
                  <div>
                    <p className="text-[10px] text-[var(--text-muted)] uppercase tracking-wider font-bold">{label}</p>
                    <p className="font-black text-[var(--text-primary)] text-sm">{value}</p>
                  </div>
                </div>
              ))}
              ...
              {selected.rules && (
                <div className="bg-[var(--bg-card)] p-5 rounded-2xl border border-[var(--border)]">
                  ...
                  <p className="text-xs text-[var(--text-secondary)] leading-relaxed">{selected.rules}</p>
                </div>
              )}
            </div>
          </div>
