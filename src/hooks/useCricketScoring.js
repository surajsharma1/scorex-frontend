...
exports.useCricketScoring = useCricketScoring;
...
// --- The Hook ---
function useCricketScoring(initialMatchData) {
    const [state, dispatch] = (0, react_1.useReducer)(scoringReducer, { ...initialState, ...initialMatchData });
    // Expose dispatch directly, but also provide wrapped helper functions for clean UI code
    const scoreRuns = (0, react_1.useCallback)((runs, isBoundary = false) => {
        dispatch({ type: 'SCORE_RUNS', payload: { runs, isBoundary } });
    }, []);
    const scoreExtra = (0, react_1.useCallback)((type, runsOffBat, extraRuns) => {
        dispatch({ type: 'EXTRA', payload: { type, runsOffBat, extraRuns } });
    }, []);
    const takeWicket = (0, react_1.useCallback)((type, runsCompletedBeforeOut = 0) => {
        dispatch({ type: 'WICKET', payload: { type, runsCompletedBeforeOut } });
    }, []);
    const undoLastBall = (0, react_1.useCallback)(() => {
        dispatch({ type: 'UNDO' });
    }, []);
    return {
        matchState: state,
...
//# sourceMappingURL=useCricketScoring.js.map