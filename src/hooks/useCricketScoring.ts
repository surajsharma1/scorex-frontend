...
export interface PlayerStats {
  ...
  fours: number;
  sixes: number;
  wickets?: number; // For bowlers
  overs?: number;   // For bowlers (stored as valid balls, e.g., 12 = 2.0 overs)
}
...
export interface MatchState {
  ...
  bowler: PlayerStats;
  currentOverLog: string[]; // e.g., ['1', 'W', 'WD', '4']
  history: Omit<MatchState, 'history'>[]; // Stack for Undo functionality
}

export type Action =
  | { type: 'SCORE_RUNS'; payload: { runs: number; isBoundary?: boolean } }
  | { type: 'EXTRA'; payload: { type: ExtraType; runsOffBat: number; extraRuns: number } }
  | { type: 'WICKET'; payload: { type: WicketType; runsCompletedBeforeOut: number } }
  | { type: 'SWAP_STRIKE' }
  | { type: 'END_OVER'; payload: { nextBowler: PlayerStats } }
  | { type: 'CHANGE_BATSMAN'; payload: { newBatsman: PlayerStats } }
  | { type: 'UNDO' };
...
function scoringReducer(state: MatchState, action: Action): MatchState {
  // Helper to save current state to history before mutating
  const saveHistory = (): Omit<MatchState, 'history'> => {
...
// --- The Hook ---
export function useCricketScoring(initialMatchData?: Partial<MatchState>) {
  ...
  const scoreRuns = useCallback((runs: number, isBoundary = false) => {
    dispatch({ type: 'SCORE_RUNS', payload: { runs, isBoundary } });
  }, []);

  const scoreExtra = useCallback((type: ExtraType, runsOffBat: number, extraRuns: number) => {
    dispatch({ type: 'EXTRA', payload: { type, runsOffBat, extraRuns } });
  }, []);

  const takeWicket = useCallback((type: WicketType, runsCompletedBeforeOut = 0) => {
